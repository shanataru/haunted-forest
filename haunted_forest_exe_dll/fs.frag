...
void main()
{
	...
	{
        float fogMode = 0.0;
        fogMode = exp(-pow(fogDensity * abs(gl_FragCoord.z / gl_FragCoord.w), 2.0f)); //gl_FragCoord - contains the window-relative coordinates of the current fragment
        fogMode = 1.0f - clamp(fogMode, 0.0f, 1.0f); //fogMode <0,1>
